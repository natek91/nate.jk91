...
*
* Name: JuAn Kim    Student ID: 131549164    Date: Feb.15th.2018
*
********************************************************************************/
...
// Array Properties (including customers,addresses,stores)
var CustomerDB = {
	customers: [],
	addresses: [],
	stores: [],
	...
	addCustomer: function(customerObj){
		...
		customerObj.add_date = todayDate;
	...
	outputCustomerById: function(customer_id){
		...
		for (var i = 0; i < this.customers.length; i++) {
			...
			+ "\n" + "Home Address: " + this.getAddressById(this.customers[i].address_id) + "Joined: " + this.customers[i].add_date + "\n\n");
			}
		}
	},
	...
	removeCustomerById: function(customer_id){
		...
		for (var i = 0; i < this.customers.length; i++){
			...
			if(this.customers[i].customer_id == customer_id){
				removeItem = this.customers[i].address_id;
				this.customers.splice(i,1);
			}
		} 
		this.removeAddressById(removeItem);
	},
	...
	getAddressById: function(address_id){
		...
		for (var i = 0; i < this.addresses.length; i++){
			if(address_id == this.addresses[i].address_id){
				add = this.addresses[i].address + " " + this.addresses[i].city + ", " + this.addresses[i].province + ". " + this.addresses[i].postal_code + "\n";
			}
		}
		return add;
	},
	...
	removeAddressById: function(address_id){
		...
		for (var j = 0; j < this.stores.length; j++){
			if(this.stores[j].address_id == address_id){
				storeB++;
			}
		}
	
		if (customerA == 0 && storeB == 0){
			for (var l = 0; l < this.addresses.length; l++){
				if(this.addresses[l].address_id == address_id){
					this.addresses.splice(l,1);
				}
			}
		}
	},
